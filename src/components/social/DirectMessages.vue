<template>
  <div class="direct-messages">
    ...
    >
      <div
        class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-4xl h-[600px] mx-4 overflow-hidden"
      >
        <!-- Header -->
        <div
        ...
        <div class="flex h-[536px]">
          <!-- Conversations List -->
          <div
            class="w-1/3 border-r border-gray-200 dark:border-gray-700 flex flex-col"
          >
            <!-- Search & New Chat -->
            ...
            <div class="flex-1 overflow-y-auto">
              ...
              >
                <div class="flex items-center gap-3">
                  <!-- User Avatar -->
                  <div
                    class="w-10 h-10 rounded-full bg-gradient-to-br from-primary-400 to-purple-500 flex items-center justify-center text-white font-bold"
                  >
                    {{
          ...
          <div class="flex-1 flex flex-col">
            ...
            <div v-else class="flex flex-col h-full">
              ...
              <div class="p-4 border-b border-gray-200 dark:border-gray-700">
                <div class="flex items-center gap-3">
                  <div
                    class="w-8 h-8 rounded-full bg-gradient-to-br from-primary-400 to-purple-500 flex items-center justify-center text-white font-bold text-sm"
                  >
                    {{
                  ...
                  <div>
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-400">
                      {{
                        selectedConversation.otherUser.wallet_address.slice(
                          0,
                          8,
                        )
                      }}...
...
const loadConversations = async () => {
  try {
    const result = await SupabaseService.getUserConversations(
      authStore.user!.id,
    );
...
const selectConversation = async (conversation: Conversation) => {
  ...
  await SupabaseService.markConversationAsRead(
    conversation.id,
    authStore.user!.id,
  );
...
const loadMessages = async (conversationId: string) => {
  try {
    messages.value =
      await SupabaseService.getConversationMessages(conversationId);
  } catch (error) {
...
const findOrCreateConversation = async (
  ...
  try {
    ...
    if (!conversation) {
      ...
      const newConversation = await SupabaseService.createConversation(
        authStore.user!.id,
        user.id,
      );
      if (newConversation) {
