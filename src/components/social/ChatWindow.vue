  ...
  <div class="flex flex-col h-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg">
      ...
      <div class="flex items-center">
        ...
        <div>
          ...
          <p class="text-xs text-gray-500 dark:text-gray-400">
            {{ participant?.walletAddress && formatWalletAddress(participant.walletAddress) }}
          </p>
        </div>
      </div>
    ...
    >
      ...
      <div v-if="loading" class="flex items-center justify-center py-8">
        ...
        <span class="ml-2 text-sm text-gray-500 dark:text-gray-400">Loading messages...</span>
      </div>
      ...
      <div v-if="!loading && messages.length === 0" class="text-center py-8">
        <Icon name="message-circle" class="w-12 h-12 text-gray-400 mx-auto mb-2" />
        <p class="text-sm text-gray-500 dark:text-gray-400">No messages yet</p>
        <p class="text-xs text-gray-400 dark:text-gray-500">Send a message to start the conversation</p>
      </div>
    </div>
...
const loadMessages = async () => {
  ...
  try {
    ...
    messages.value = [
      ...
      {
        id: '2', 
        content: 'Thanks! Been working on it for weeks. What do you think of the tokenomics?',
...
const sendMessage = async () => {
  ...
  try {
    ...
    const sentMessage = messages.value.find(m => m.id === optimisticMessage.id)
    if (sentMessage) {
