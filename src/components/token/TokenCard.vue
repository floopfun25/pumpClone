  ...
  >
      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center space-x-2 md:space-x-2">
          ...
          <div class="min-w-0 flex-1">
            <h3
              class="text-base md:text-base font-semibold text-white truncate"
            >
              {{ token.name }}
            </h3>
            <p class="text-sm md:text-xs text-binance-gray">
              {{ token.symbol }}
            </p>
          </div>
        </div>
        <div class="text-right flex-shrink-0">
          <p class="text-base md:text-sm font-medium text-binance-yellow">
    ...
    <div class="px-3 pb-3 md:px-3 md:pb-3 space-y-2 md:space-y-1">
      ...
      <div class="flex justify-between text-sm md:text-xs">
        ...
        <span class="text-white font-medium"
          >${{ formatNumber(token.volume24h) }}</span
        >
      </div>
      <div class="flex justify-between text-sm md:text-xs">
    ...
    <div class="md:hidden absolute bottom-2 right-2 opacity-30">
      <svg
        class="h-4 w-4 text-binance-gray"
        fill="none"
        stroke="currentColor"
        viewBox="0 0 24 24"
      >
