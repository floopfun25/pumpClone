<template>
  <div v-if="kingToken" class="king-of-hill-container mb-8">
    ...
    <div class="text-center mb-6">
      <div
        class="inline-flex items-center bg-gradient-to-r from-yellow-400 to-orange-500 text-black px-6 py-3 rounded-full text-lg font-bold shadow-lg"
      >
        👑 {{ $t("token.kingOfHill") }} 👑
      </div>
      <p class="text-gray-400 text-sm mt-2">
        {{ $t("token.kingOfHillDescription") }}
      </p>
    </div>

    <!-- King Token Card -->
    <div
      class="king-token-card relative overflow-hidden rounded-2xl bg-gradient-to-br from-yellow-500/20 via-orange-500/20 to-red-500/20 border-2 border-yellow-400/50 shadow-2xl"
    >
      <!-- Animated Background -->
      <div
        class="absolute inset-0 bg-gradient-to-r from-yellow-400/10 to-orange-500/10 animate-pulse"
      ></div>

      <!-- Crown Badge -->
      <div
        class="absolute top-4 right-4 bg-yellow-400 text-black px-3 py-1 rounded-full text-sm font-bold animate-bounce"
      >
        👑 KING
      </div>
      ...
      <div class="relative z-10 p-8">
        <div class="flex flex-col lg:flex-row items-center gap-8">
          ...
          <div class="flex items-center gap-6">
            <div
              class="w-24 h-24 rounded-full overflow-hidden ring-4 ring-yellow-400/50"
            >
              <img
              ...
              <div
                v-else
                class="w-full h-full bg-gradient-to-br from-yellow-400 to-orange-500 flex items-center justify-center text-black font-bold text-3xl"
              >
            ...
            <div>
              <h3
                class="text-3xl font-bold text-white mb-2 flex items-center gap-2"
              >
                {{ kingToken.name }}
          ...
          <div class="flex-1 lg:ml-8">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
              ...
              <div class="text-center">
                <div class="text-2xl font-bold text-green-400 mb-1">
                  ${{ formatMarketCap(kingToken.market_cap || 0) }}
                </div>
                <div class="text-sm text-gray-400 uppercase tracking-wide">
                  {{ $t("token.marketCapShort") }}
                </div>
              </div>
            ...
            <div class="mt-6">
              ...
              <div class="flex justify-between text-xs text-gray-500 mt-1">
                <span>${{ formatNumber(kingToken.market_cap || 0) }}</span>
        ...
        <!-- Creator Info -->
        <div
          v-if="kingToken.creator"
          class="mt-6 pt-6 border-t border-yellow-400/30"
        >
          <div class="flex items-center gap-3">
            <span class="text-gray-400">{{ $t("token.createdBy") }}</span>
            <span class="text-yellow-400 font-semibold">
              {{
                kingToken.creator.username ||
                `${kingToken.creator.wallet_address.slice(0, 6)}...${kingToken.creator.wallet_address.slice(-4)}`
              }}
            </span>
    ...
    <div class="text-center mt-4">
      <span
        class="inline-flex items-center bg-red-500/20 text-red-400 px-4 py-2 rounded-full text-sm font-semibold animate-pulse"
      >
        🔥 {{ $t("token.hot") }} 🔥
      </span>
    </div>
