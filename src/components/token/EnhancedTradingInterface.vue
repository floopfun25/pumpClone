    ...
    <div class="p-6 pb-4">
      ...
      <div class="mb-6">
        ...
        <div v-else class="text-right">
          <span class="text-sm font-medium text-gray-600 dark:text-gray-400">{{ slippageTolerance }}%</span>
        </div>
      </div>
...
const calculateTradePreview = async () => {
  ...
  try {
    ...
    const amountBigInt = BigInt(Math.floor(amount * (tradeType.value === 'buy' ? 1e9 : 1e6))) // Convert to lamports/tokens

    if (tradeType.value === 'buy') {
      tradePreview.value = BondingCurveService.calculateBuy(props.bondingCurveState, amountBigInt)
    } else {
      tradePreview.value = BondingCurveService.calculateSell(props.bondingCurveState, amountBigInt)
    }
  } catch (error) {
...
const handleCustomSlippageBlur = () => {
  if (customSlippage.value && customSlippage.value >= 0.1 && customSlippage.value <= 50) {
    slippageTolerance.value = customSlippage.value
  }
}

// Watchers
watch(() => tradeAmount.value, () => {
  calculateTradePreview()
})
