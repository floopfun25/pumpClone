<template>
  <div
    class="creator-incentives bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6"
  >
    <!-- Header -->
    ...
    <div v-if="authStore.isAuthenticated" class="mb-8">
      <div
        class="bg-gradient-to-r from-primary-50 to-purple-50 dark:from-primary-900/20 dark:to-purple-900/20 rounded-lg p-6"
      >
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-4">
            <!-- Creator Badge -->
            <div
              class="w-16 h-16 rounded-full bg-gradient-to-br from-primary-400 to-purple-500 flex items-center justify-center"
            >
              <span class="text-white text-2xl">{{
                getCreatorBadge(creatorData?.tier || "bronze")
              }}</span>
            </div>
            ...
            <div>
              ...
              <p class="text-gray-600 dark:text-gray-400">
                Level {{ creatorData?.level || 1 }} •
                {{ creatorData?.tokensCreated || 0 }} tokens created
              </p>
              ...
              <div class="mt-3">
                ...
                >
                  <span
                    >Progress to
                    {{
                      getNextTier(creatorData?.tier || "bronze").toUpperCase()
                    }}</span
                  >
        ...
        <div class="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p class="text-sm text-gray-600 dark:text-gray-400">
              Earn SOL for creating successful tokens
            </p>
          </div>
        </div>
        ...
        <div class="space-y-3">
          <div class="flex justify-between items-center">
            ...
            <span class="font-medium text-gray-900 dark:text-white">{{
              creatorData?.tokensCreated || 0
            }}</span>
          </div>
          <div class="flex justify-between items-center">
            <span class="text-sm text-gray-600 dark:text-gray-400"
              >Graduation Bonus</span
            >
            <span class="font-medium text-pump-green">{{
              formatSOL(creatorData?.graduationBonus || 0)
            }}</span>
          </div>
          <div class="flex justify-between items-center">
            <span class="text-sm text-gray-600 dark:text-gray-400"
            ...
            >
            <span class="font-medium text-pump-green">{{
              formatSOL(creatorData?.volumeRewards || 0)
            }}</span>
          </div>
        </div>
      </div>
      ...
      >
        ...
        <div class="space-y-3">
          ...
          <div class="flex justify-between items-center">
            ...
            >
            <span class="font-medium text-gray-900 dark:text-white">{{
              creatorData?.helpfulVotes || 0
            }}</span>
          </div>
          <div class="flex justify-between items-center">
            <span class="text-sm text-gray-600 dark:text-gray-400"
              >Engagement Rewards</span
            >
            <span class="font-medium text-pump-green">{{
              formatSOL(creatorData?.engagementRewards || 0)
            }}</span>
          </div>
        </div>
      </div>
    ...
    <div v-if="authStore.isAuthenticated">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        ...
        >
          ...
          <!-- Progress Bar -->
          <div
            class="w-full bg-gray-200 dark:bg-gray-600 rounded-full h-2 mb-4"
          >
            <div
              :class="[
                'h-2 rounded-full transition-all duration-300',
                program.progressColor,
              ]"
    ...
    <div v-if="authStore.isAuthenticated">
      ...
      >
        ...
        >
          ...
          <!-- Earnings -->
          <div class="text-right">
            <div class="font-bold text-pump-green">
              {{ formatSOL(creator.earnings) }}
            </div>
...
const getProgressToNextTier = (): number => {
  ...
  if (nextTier === "diamond" && creatorData.value.tier === "diamond")
    return 100;
