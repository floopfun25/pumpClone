<template>
  <div class="advanced-chart-container">
    ...
    <!-- Chart Container -->
    <div
      class="relative bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700"
    >
      <!-- Loading Overlay -->
      <div
        v-if="loading"
      ...
      >
        <div class="flex flex-col items-center">
          <div
            class="animate-spin rounded-full h-8 w-8 border-b-2 border-primary-500 mb-2"
          ></div>
      ...
      <div v-else class="chart-wrapper p-4">
        ...
        >
          ...
          <div v-if="tooltip.volume" class="text-gray-300">
    ...
    <div
      v-if="hasData"
      class="grid grid-cols-2 md:grid-cols-6 gap-4 mt-4 p-4 bg-gray-50 dark:bg-gray-800/50 rounded-lg"
    >
      <div class="text-center">
...
const drawTimeLabels = (
  ...
  for (let i = 0; i <= labelCount; i++) {
    const dataIndex = Math.floor(
      (i * (chartData.value.length - 1)) / labelCount,
    );
...
const handleMouseMove = (event: MouseEvent) => {
  ...
  if (x >= padding && x <= rect.width - padding) {
    const dataIndex = Math.round(
      ((x - padding) / chartWidth) * (chartData.value.length - 1),
    );
...
const setupRealTimeUpdates = () => {
  ...
  priceSubscription = priceOracleService.subscribe(
    props.mintAddress,
    (priceData: any) => {
      ...
      if (lastCandle && newCandle.time - lastCandle.time < 300000) {
        // Within 5 minutes
