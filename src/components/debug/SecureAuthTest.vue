<template>
  <div class="secure-auth-test p-6 bg-white dark:bg-gray-800 rounded-lg shadow-lg">
    ...
    <div class="space-y-4">
      ...
      <div class="status-card p-4 rounded-lg border">
        ...
        <div class="space-y-2">
          <div class="flex items-center gap-2">
            ...
            <span :class="walletStore.isConnected ? 'text-green-600' : 'text-red-600'">
          ...
          <div v-if="walletStore.isConnected" class="flex items-center gap-2">
            ...
            <span class="text-sm font-mono">{{ walletStore.walletAddress }}</span>
          </div>
          <div class="flex items-center gap-2">
      ...
      <div class="test-steps space-y-3">
        ...
        <div class="step-card p-3 rounded border" :class="walletStore.isConnected ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-200'">
          <div class="flex items-center justify-between">
            <span class="font-medium">1. Connect Wallet</span>
            <span v-if="walletStore.isConnected" class="text-green-600">✅</span>
          </div>
          <button 
        ...
        <div class="step-card p-3 rounded border" :class="challenge ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-200'">
          <div class="flex items-center justify-between">
            <span class="font-medium">2. Generate Authentication Challenge</span>
        ...
        <div class="step-card p-3 rounded border" :class="signature ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-200'">
          <div class="flex items-center justify-between">
        ...
        <div class="step-card p-3 rounded border" :class="authStore.isAuthenticated ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-200'">
          <div class="flex items-center justify-between">
            <span class="font-medium">4. Verify Signature & Authenticate</span>
            <span v-if="authStore.isAuthenticated" class="text-green-600">✅</span>
          </div>
          <button 
      ...
      <div v-if="logs.length > 0" class="logs-section mt-6">
        ...
        <div class="max-h-40 overflow-y-auto bg-black text-green-400 p-4 rounded font-mono text-sm">
...
const generateChallenge = async () => {
  try {
    ...
    challenge.value = SecureAuthService.generateChallenge(walletStore.walletAddress)
    log(`Challenge generated: ${challenge.value.challenge.slice(0, 10)}...`)
    
  } catch (err) {
    error.value = err instanceof Error ? err.message : 'Challenge generation failed'
