    ...
    <div v-else class="error-boundary">
      <div class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-6 text-center">
        ...
        <p class="text-red-600 dark:text-red-300 mb-4 text-sm">
          {{ message || 'This component encountered an error and couldn\'t be displayed properly.' }}
        </p>
        
        <!-- Development Mode Error Details -->
        <div v-if="isDev && errorDetails" class="mt-4 p-3 bg-red-100 dark:bg-red-900/40 rounded border text-left">
          <details>
            <summary class="cursor-pointer text-sm font-medium text-red-700 dark:text-red-200 mb-2">
              Error Details (Development Mode)
            </summary>
            <pre class="text-xs text-red-600 dark:text-red-300 overflow-auto">{{ errorDetails }}</pre>
          </details>
        </div>
        ...
        <!-- Fallback Content -->
        <div v-if="$slots.fallback" class="mt-6 pt-4 border-t border-red-200 dark:border-red-700">
          <slot name="fallback" />
        </div>
      </div>
    </div>
