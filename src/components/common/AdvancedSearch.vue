    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
      ...
      <div>
        ...
        <select v-model="filters.sortBy" class="input-field text-sm">
          ...
          <option value="bonding_curve_progress">
            {{ t("search.progress") }}
          </option>
        </select>
      </div>
          ...
          <span class="text-sm text-gray-600 dark:text-gray-400">
            {{
              searching
                ...
                : t("search.foundTokens", { count: resultCount })
            }}
          </span>
          <span
            v-if="searchQuery"
...
const activeFiltersDisplay = computed(() => {
  ...
  if (filters.value.marketCapRange)
    active.push(`Market Cap: ${filters.value.marketCapRange}`);
  if (filters.value.ageRange) active.push(`Age: ${filters.value.ageRange}`);
  if (filters.value.volumeRange)
    active.push(`Volume: ${filters.value.volumeRange}`);
