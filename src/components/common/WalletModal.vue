    ...
    <div class="flex min-h-full items-center justify-center p-4">
      ...
      >
        ...
        <div class="flex items-center justify-between mb-6 md:mb-6 mb-4">
          <h2
            class="text-xl md:text-xl text-lg font-semibold text-gray-900 dark:text-white"
          >
        ...
        <div v-if="connecting" class="text-center py-8">
          ...
          <p
            v-if="isMobileDevice && selectedWallet"
        ...
        <div v-if="!connecting" class="space-y-4 md:space-y-3">
          ...
          <div v-if="displayWallets.length > 0">
            <h3
              class="text-base md:text-sm font-medium text-gray-700 dark:text-gray-300 mb-4 md:mb-3"
            >
              {{ t("wallet.selectWallet") }}
            </h3>
            <div class="space-y-3 md:space-y-2">
              ...
              >
                ...
                <div class="flex-1 text-left">
                  <p
                    class="font-medium text-gray-900 dark:text-white text-base md:text-sm"
                  >
                    {{ wallet.name }}
                  </p>
                </div>
                <svg
                  class="w-6 h-6 md:w-5 md:h-5 text-gray-400"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
          ...
          <div
            v-if="notInstalledWallets.length > 0 && !isMobileDevice"
            class="mt-6"
          >
            <h3
              class="text-base md:text-sm font-medium text-gray-700 dark:text-gray-300 mb-4 md:mb-3"
          ...
          <div v-if="displayWallets.length === 0" class="text-center py-8">
            <div class="text-gray-400 mb-4">
              <svg
                class="w-16 h-16 md:w-12 md:h-12 mx-auto"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
            ...
            <p
              class="text-gray-600 dark:text-gray-400 mb-4 text-base md:text-sm"
            >
              {{ t("wallet.noWallet") }}
            </p>
        ...
        >
          <div class="flex items-start space-x-2">
            <svg
              class="w-6 h-6 md:w-5 md:h-5 text-red-500 mt-0.5 flex-shrink-0"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
            ...
            <div class="flex-1">
              <h4
                class="text-base md:text-sm font-medium text-red-800 dark:text-red-200 mb-1"
              >
                {{ t("messages.error.walletConnection") }}
              </h4>
              <p
                class="text-sm md:text-xs text-red-600 dark:text-red-400 whitespace-pre-wrap break-words"
              >
                {{ error }}
              </p>
            </div>
        ...
        <div class="mt-6 pt-4 border-t border-gray-200 dark:border-gray-600">
          <p
            class="text-sm md:text-xs text-gray-500 dark:text-gray-400 text-center"
          >
            {{ t("walletModal.termsAgreement", { appName: t("app.name") }) }}
            <a
              href="/terms"
              class="text-primary-600 dark:text-primary-400 hover:underline"
              >{{ t("walletModal.termsOfService") }}</a
            >
          </p>
        </div>
      </div>
...
const notInstalledWallets = computed(() => {
  ...
  return allWallets.value.filter(
    (wallet) => !availableWallets.value.find((w) => w.name === wallet.name),
  );
});
