...
export class RealSolanaProgram {
  ...
  async buyTokens(
    ...
    try {
      ...
      const [bondingCurveAccount] = await this.getBondingCurveAddress(mintAddress)
      ...
      const instructionData = borsh.serialize(
        ...
        new BondingCurveInstruction(1, borsh.serialize(BONDING_CURVE_SCHEMA, buyInstructionData))
      )
      ...
      // Update database
      await this.recordTrade('buy', signature, mintAddress.toBase58(), buyer.toBase58(), solAmountLamports, tokensTraded)
  ...
  async sellTokens(
    ...
    try {
      ...
      const tokenAccountInfo = await getAccount(this.connection, sellerTokenAccount)
      ...
      const instructionData = borsh.serialize(
        BONDING_CURVE_SCHEMA,
        new BondingCurveInstruction(2, borsh.serialize(BONDING_CURVE_SCHEMA, sellInstructionData))
      )
      ...
      // Update database
      await this.recordTrade('sell', signature, mintAddress.toBase58(), seller.toBase58(), solAmount, tokenAmount)
   ...
   */
  async getBondingCurveAddress(mintAddress: PublicKey): Promise<[PublicKey, number]> {
  ...
  async getMarketCap(mintAddress: PublicKey): Promise<number> {
    try {
      ...
      return Number(mintInfo.supply) * price / Math.pow(10, mintInfo.decimals)
    } catch (error) {
      return 0
    }
  }
