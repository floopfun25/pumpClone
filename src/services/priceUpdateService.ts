...
class PriceUpdateService {
  private static subscribers = new Map<string, Set<(update: PriceUpdate) => void>>()
   ...
   */
  static subscribe(tokenId: string, callback: (update: PriceUpdate) => void): () => void {
  ...
  private static async fetchAndBroadcastPrice(tokenId: string) {
    try {
      ...
      const priceChange24h = previousPrice ? ((token.current_price - previousPrice) / previousPrice) * 100 : 0
   ...
   */
  static createPriceRef(tokenId: string, initialPrice: number = 0): {
    price: Ref<number>
