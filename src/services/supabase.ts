...
const { createClient } =
  (window as any).__SUPABASE_CDN__ || require("@supabase/supabase-js");
  ...
  public: {
    Tables: {
      ...
      token_comments: {
        ...
        Update: Partial<
          Omit<
            Database["public"]["Tables"]["token_comments"]["Row"],
            "id" | "created_at"
          >
        >;
      };
...
function createSupabaseClient(): SupabaseClient<Database> {
  ...
  try {
    ...
    const clientFactory =
      (window as any).__SUPABASE_CDN__?.createClient || createClient;
    ...
    try {
      ...
      if (existingUser) {
        ...
        if (currentSession && currentSession.user?.id === existingUser.id) {
          console.log(
            "✅ Session matches existing user. Authentication complete.",
          );
        ...
        if (currentSession) {
          console.warn(
            `Mismatched session found. Signing out before re-authenticating.`,
          );
        ...
        console.log(
          "🆕 Creating a new auth session and migrating existing profile to it...",
        );
        ...
        if (migrateError) {
          ...
          await supabase.auth.signOut();
          throw new Error(
            `Failed to reconcile user accounts: ${migrateError.message}`,
          );
        }
        ...
        if (!finalSession)
          throw new Error("Failed to get session after user migration.");

        console.log(
          `✅ Successfully migrated profile for ${existingUser.wallet_address}`,
        );
        return {
          user: {
            ...finalSession.user,
      ...
      if (anonError || !anonData.user) {
        throw new Error(
          `Supabase anonymous sign-in failed: ${anonError?.message}`,
        );
      }
      ...
      return {
        user: { ...anonData.user, ...newUserProfile },
...
export class SupabaseService {
  ...
  static async upsertUser(
    ...
    try {
      if (!userData.wallet_address)
        throw new Error("Wallet address is required to upsert a user.");
      ...
      if (error) {
        ...
        if (error.code === "23505") {
          // unique_violation
          console.warn(
            "Upsert failed due to conflict, fetching existing user.",
          );
  ...
  static async getTokens(
    ...
    try {
      ...
      if (search) {
        query = query.or(
          `name.ilike.%${search}%,symbol.ilike.%${search}%,description.ilike.%${search}%`,
        );
      }
  ...
  static async updateToken(
    tokenId: string,
    updateData: Partial<Database["public"]["Tables"]["tokens"]["Update"]>,
  ) {
    try {
      const { data, error } = await supabase
        .from("tokens")
  ...
  static subscribeToTransactions(
      ...
      const pollTransactions = async () => {
        try {
          ...
          const newTransactions = transactions.filter(
            (tx) => new Date(tx.created_at).getTime() > lastTransactionTime,
          );

          if (newTransactions.length > 0) {
            // Update last transaction time
            lastTransactionTime = Math.max(
              ...newTransactions.map((tx) => new Date(tx.created_at).getTime()),
            );
    ...
    try {
      console.log(
        "🔔 Attempting realtime transaction subscription for:",
        tokenId,
      );
      ...
      const channel = supabase
        ...
        .subscribe((status, err) => {
          ...
          if (status === "SUBSCRIBED") {
            ...
            console.log(
              "⚠️ Realtime unavailable, falling back to transaction polling",
            );
            isRealtimeActive = false;
      ...
      console.error(
        "❌ Realtime failed, using transaction polling mode:",
        error,
      );
      startTransactionPolling();
      return {
  ...
  static async getDashboardStats() {
    try {
      ...
      const totalVolume =
        volumeData?.reduce((sum, tx) => sum + (tx.sol_amount || 0), 0) || 0;
  ...
  static async getUserPortfolio(userId: string) {
    try {
      ...
      const portfolioItems = holdings.map((holding: any) => {
        const tokenValue =
          (holding.amount / Math.pow(10, holding.token.decimals)) *
          holding.token.current_price;
   ...
   * Used for chart visualization
   */
  static async storeTokenPriceData(
    tokenId: string,
    priceData: {
      price: number;
      volume?: number;
  ...
  static async getMarketStats(): Promise<{
    ...
    try {
      const [tokensResult, volumeResult, tradersResult, allTokens] =
        await Promise.all([
      ...
      const totalVolume24h =
        volumeResult.data?.reduce(
          (sum, transaction) => sum + (transaction.sol_amount || 0),
          0,
        ) || 0; // Already in SOL

      // Calculate real market cap based on each token's supply and price
      const totalMarketCap =
        allTokens.data?.reduce((sum, token) => {
    ...
    try {
      ...
      if (
        ...
        if (filter === "featured" && fallbackData.length > 0) {
          return this.processTrendingTokens(
            fallbackData.map((token, index) => ({
              ...token,
  ...
  private static processTrendingTokens(tokens: any[]): any[] {
    return tokens.map((token: any) => ({
      ...
      bonding_curve_progress:
        ...
        Math.min((token.market_cap / 69000) * 100, 100),
    }));
  }
  ...
  static async searchTokens(options: {
    ...
    try {
      ...
      let supabaseQuery = supabase.from("tokens").select(
        `
          *,
          creator:users(id, username, wallet_address)
        `,
      ...
      if (filters.marketCapRange) {
        switch (filters.marketCapRange) {
          ...
          case "small":
            supabaseQuery = supabaseQuery
              .gte("market_cap", 10000)
              .lt("market_cap", 100000);
            break;
          case "medium":
            supabaseQuery = supabaseQuery
              .gte("market_cap", 100000)
              .lt("market_cap", 1000000);
            break;
          case "large":
  ...
  static async getKingOfTheHillToken() {
    try {
      ...
      const eligibleTokens = (tokens || [])
        .filter((token) => {
          const commentCount = Array.isArray(token.comments)
            ? token.comments.length
            : token.comments?.count || 0;
  ...
  static calculateKingScore(token: any): number {
    ...
    const ageInMinutes =
      (Date.now() - new Date(token.created_at).getTime()) / (1000 * 60);
  ...
  static async checkTokenGraduation(tokenId: string) {
    try {
      ...
      if (
        token.market_cap >= GRADUATION_THRESHOLD &&
        token.status === "active"
      ) {
  ...
  static async getBondingCurveProgress(tokenId: string) {
    try {
      ...
      const progress = Math.min(
        (totalSolRaised / GRADUATION_THRESHOLD_SOL) * 100,
        100,
      );
  ...
  static calculateTrendingScore(token: any): number {
    ...
    const ageInHours =
      (Date.now() - new Date(token.created_at).getTime()) / (1000 * 60 * 60);
    const recencyBonus =
      ageInHours < 24 ? Math.max(0, 100 - ageInHours * 4) : 0;
  ...
  static async updateUserProfile(
    userId: string,
    updateData: { username?: string; bio?: string },
  ) {
    try {
      const { data, error } = await supabase
        .from("users")
  ...
  static async getTokensByCreator(userId: string) {
    try {
      ...
      console.log(
        `✅ Found ${transformedData.length} tokens for user ${userId}:`,
        transformedData,
      );
  ...
  static async addToWatchlist(userId: string, tokenId: string) {
    try {
      ...
      if (!tableExists) {
        throw new Error(
          "Watchlist feature is not yet available. Please run the database migration.",
        );
      }
   ...
   */
  static async isTokenInWatchlist(
    userId: string,
    tokenId: string,
  ): Promise<boolean> {
    try {
  ...
  static async getTokenTopHolders(tokenId: string, limit = 20) {
    try {
      ...
      const totalHoldings =
        data?.reduce((sum, holding) => sum + holding.amount, 0) || 1;
      ...
      const topHolders =
        data?.map((holding) => {
          ...
          if (percentage > 100) {
            console.warn(
              "⚠️ Percentage calculation error, falling back to safer calculation",
            );
          ...
          return {
            address: user?.wallet_address
              ? this.formatWalletAddress(user.wallet_address)
              : "Unknown",
  ...
  static async debugUserTokens(walletAddress: string) {
    try {
      console.log(
        "🔍 Debug: Starting user token investigation for wallet:",
        walletAddress,
      );
   ...
   */
  static async fixDuplicateUser(
    correctWalletAddress: string,
    ...
    try {
      ...
      console.log(
        `✅ Updated ${updatedTokens?.length || 0} tokens to correct user`,
      );
      ...
      const { error: userUpdateError } = await supabase
        .from("users")
        .update({
          tokens_created: (correctUser.tokens_created || 0) + tokenCount,
        })
   ...
   */
  static async getUserShareCount(
    userId: string,
    contentUrl: string,
  ): Promise<number> {
    try {
