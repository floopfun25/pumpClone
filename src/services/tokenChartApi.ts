...
export async function fetchTokenChartDataWithFallback(
  ...
  // Try Birdeye
  try {
    const birdeyeData = await retryFetch(() =>
      fetchBirdeyeChartData(tokenId, interval),
    );
    if (birdeyeData.length > 0) return birdeyeData;
    errorLog.push("Birdeye returned no data");
  } catch (err) {
    errorLog.push(
      `Birdeye failed: ${err instanceof Error ? err.message : String(err)}`,
    );
  ...
  try {
    ...
    const coingeckoData = await retryFetch(() =>
      fetchCoinGeckoChartData(tokenSymbol.toLowerCase(), days),
    );
    if (coingeckoData.length > 0) return coingeckoData;
    errorLog.push("CoinGecko returned no data");
  } catch (err) {
    errorLog.push(
      `CoinGecko failed: ${err instanceof Error ? err.message : String(err)}`,
    );
...
export async function fetchBirdeyeChartData(
  tokenAddress: string,
  interval: string = "1h",
) {
  try {
...
export async function fetchCoinGeckoChartData(
  tokenId: string,
  days: number = 1,
) {
  try {
