...
const safePrice = (
  price: number,
  fallbackPrice: number = 0.0000001,
): number => {
  if (!Number.isFinite(price) || price <= 0 || price > 1000000) {
    console.warn(
      "Invalid price calculated, using fallback:",
      price,
      "-> fallback:",
      fallbackPrice,
    );
...
const safeMarketCap = (price: number, totalSupply: number): number => {
  const marketCap = price * totalSupply; // Market cap in SOL, will be converted to USD elsewhere
...
export class SolanaProgram {
  ...
  async getBondingCurveAccount(
    ...
    try {
      ...
      const totalSupply = BigInt(
        token.total_supply * Math.pow(10, token.decimals),
      );

      // Use bonding curve config for initial state
      const initialVirtualSol = BigInt(
        bondingCurveConfig.initialVirtualSolReserves * LAMPORTS_PER_SOL,
      );
      const initialVirtualTokens = BigInt(
      ...
      const virtualSolReserves =
        initialVirtualSol +
        BigInt(Math.floor(totalSolRaised * LAMPORTS_PER_SOL));
      ...
      return {
        ...
        virtualSolReserves,
        realTokenReserves: BigInt(
          token.locked_tokens_amount * Math.pow(10, token.decimals),
        ),
  ...
  async initializeBondingCurve(
    ...
    const [bondingCurveAddress, bumpSeed] =
      this.deriveBondingCurveAddress(mintAddress);
  ...
  async buyTokens(
    ...
    try {
      ...
      if (sessionError || !session?.user?.id) {
        throw new Error(
          "User not authenticated. Please sign in with your wallet first.",
        );
      }
      ...
      try {
        ...
        if (ensureError) {
          console.error(
            "❌ [TRADING] Failed to ensure user exists:",
            ensureError,
          );
        ...
        console.log(
          "✅ [TRADING] User record verified/created:",
          ensuredUserId,
        );
      } catch (rpcError) {
        console.error(
          "❌ [TRADING] RPC call failed, trying direct user creation...",
        );
        ...
        if (!existingUser) {
          ...
          if (createError) {
            console.error(
              "❌ [TRADING] Failed to create user record:",
              createError,
            );
      ...
      const calculatedPrice =
        Number(newSolReserves) / Number(newTokenReserves) / LAMPORTS_PER_SOL;
      const newPrice = safePrice(
        calculatedPrice,
        token.current_price || 0.0000001,
      );
      ...
      const { error: txError } = await supabase.from("transactions").insert({
        ...
        token_amount: safeNumber(
          Number(tokensOut) / Math.pow(10, token.decimals),
      ...
      const { error: updateError } = await supabase
        ...
        .update({
          ...
          bonding_curve_progress: Math.min(
            100,
            (newMarketCap / token.graduation_threshold) * 100,
          ),
      ...
      if (existingHolding) {
        ...
        const newTotalInvested = safeNumber(
          (existingHolding.total_invested || 0) + solAmount,
        );
        const tokenAmountInFullUnits =
          Number(newAmount) / Math.pow(10, token.decimals);
  ...
  async sellTokens(
    ...
    const tokenAmountWithDecimals = BigInt(
    ...
    const newTokenReserves =
      bondingCurve.virtualTokenReserves + tokenAmountWithDecimals;
    ...
    try {
      ...
      try {
        ...
        console.log(
          "✅ [TRADING] User record verified for selling:",
          ensuredUserId,
        );
      } catch (rpcError) {
        ...
        if (!existingUser) {
          throw new Error(
            "User record not found. Please reconnect your wallet.",
          );
        }
      }
      ...
      if (
        holdingError ||
        !holding ||
        holding.amount < Number(tokenAmountWithDecimals)
      ) {
      ...
      const { error: txError } = await supabase.from("transactions").insert({
        ...
        token_amount: safeNumber(
          Number(tokenAmountWithDecimals) / Math.pow(10, token.decimals),
        ),
        price_per_token: newPrice,
        bonding_curve_price: newPrice,
        slippage_percentage: slippagePercent,
      ...
      const { error: updateError } = await supabase
        ...
        .update({
          current_price: newPrice,
          market_cap: newMarketCap,
          volume_24h: safeNumber(
            (token.volume_24h || 0) + Number(solOut) / LAMPORTS_PER_SOL,
          ),
          last_trade_at: new Date().toISOString(),
          bonding_curve_progress: Math.min(
            100,
   ...
   */
  async getUserTokenAccounts(userPublicKey: PublicKey): Promise<
    Array<{
      mint: string;
      balance: number;
      decimals: number;
