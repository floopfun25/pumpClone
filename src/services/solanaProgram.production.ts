...
export class SolanaProgram {
  ...
  async buyTokens(
    ...
    try {
      ...
      const updatedBondingCurve =
        await this.getBondingCurveAccount(mintAddress);
      ...
      const actualTokensReceived =
        ...
        updatedBondingCurve.virtualTokenReserves;
      ...
      console.log(
        "📊 Tokens received:",
        Number(actualTokensReceived) / Math.pow(10, 9),
      );
  ...
  async sellTokens(
    ...
    try {
      ...
      const actualSolReceived =
        updatedBondingCurve.virtualSolReserves -
  ...
  private async updateDatabaseAfterBuy(
    ...
    const newPrice =
      Number(bondingCurve.virtualSolReserves) /
