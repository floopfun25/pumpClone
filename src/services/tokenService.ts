...
class TokenService {
  ...
  private async uploadImage(file: File): Promise<string> {
    try {
      ...
      if (error) {
        ...
        throw new Error(
          `Storage upload failed: ${error.message || "Unknown storage error"}`,
        );
      }
      ...
      try {
        const { testImageUrl, analyzeSupabaseUrl } = await import(
          "@/utils/imageDebug"
        );
        ...
        const imageTest = new Promise<boolean>((resolve) => {
          ...
          img.onerror = (e) => {
            console.error(
              "❌ [IMAGE UPLOAD] Image element verification failed:",
              e,
            );
            resolve(false);
          };
        ...
        if (imageValid) {
          ...
          console.error(
            "💥 [IMAGE UPLOAD] Image verification failed - check if the file is corrupted",
          );
        }

        // Debug: Log the final URL for manual testing
        console.log(
          "🔗 [IMAGE UPLOAD] Manual verification URL:",
          publicUrl.publicUrl,
        );
        console.log(
          "📋 [IMAGE UPLOAD] To test: Open this URL in a new browser tab to verify the image displays correctly",
        );
      } catch (testError) {
        console.error(
          "💥 [IMAGE UPLOAD] Upload verification failed:",
          testError,
        );
        throw new Error(
          `Image upload verification failed: ${testError instanceof Error ? testError.message : "Unknown error"}`,
        );
      }
  ...
  private async saveTokenToDatabase(data: {
    ...
    try {
      ...
      const totalSupply =
        data.tokenData.totalSupply || tokenDefaults.totalSupply;
      ...
      const creatorTokensAllocated = Math.floor(
        (totalSupply * creatorSharePercentage) / 100,
      );
      const creatorTokensLocked = Math.floor(
        (creatorTokensAllocated * lockPercentage) / 100,
      );
      const creatorTokensUnlocked =
        creatorTokensAllocated - creatorTokensLocked;
      ...
      const initialState = BondingCurveService.createInitialState(
      ...
      const calculatedMarketCap = Math.floor(
        BondingCurveService.calculateMarketCapSync(initialState, 169),
      );
      ...
      const nextUnlockDate =
        lockStartDate && lockDurationDays > 0
          ? new Date(
              lockStartDate.getTime() + lockDurationDays * 24 * 60 * 60 * 1000,
            )
          : null;
      ...
      console.log(
        "Token saved to database successfully with advanced features",
      );
    } catch (error) {
  ...
  private async createTokenLockRecord(params: {
    ...
    try {
      ...
      const { error } = await supabase.from("token_locks").insert({
      ...
      if (params.unlockSchedule === "linear" && params.lockDurationDays > 30) {
        await this.createLinearUnlockSchedule(
          params.tokenId,
          params.lockedAmount,
   ...
   */
  private async createLinearUnlockSchedule(
    lockId: string,
    totalAmount: number,
    durationDays: number,
  ) {
    try {
      ...
      for (let i = 1; i <= unlockIntervals; i++) {
        ...
        unlockSchedule.push({
          ...
          unlock_amount:
            i === unlockIntervals
              ? totalAmount - amountPerUnlock * (i - 1)
              : amountPerUnlock,
          status: "pending",
        });
      }
