...
class TokenEconomicsService {
  ...
  static calculateTokenEconomics(params: {
    ...
    const creatorShare = Math.floor((totalSupply * creatorSharePercentage) / 100)
   ...
   */
  static async getTokenEconomics(tokenId: string): Promise<TokenEconomics | null> {
    try {
      ...
      return {
        ...
        marketSupply: (token.total_supply_custom || token.total_supply) - (token.creator_tokens_allocated || 0),
  ...
  static async getTokenLocks(tokenId: string): Promise<TokenLock[]> {
    try {
      ...
      return locks.map(lock => ({
        ...
        lockType: lock.lock_type as 'creator' | 'team' | 'liquidity' | 'marketing',
        lockDurationDays: lock.lock_duration_days,
        unlockDate: new Date(lock.unlock_date),
        unlockSchedule: (lock.token_unlock_schedule || []).map((schedule: any) => ({
          id: schedule.id,
        ...
        status: lock.status as 'locked' | 'partially_unlocked' | 'fully_unlocked'
      }))
    } catch (error) {
  ...
  static async getLockedTokenValue(tokenId: string): Promise<{
    ...
    try {
      const [tokenData, locks] = await Promise.all([
        supabase.from('tokens').select('current_price').eq('id', tokenId).single(),
