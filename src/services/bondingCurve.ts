...
const INITIAL_VIRTUAL_TOKEN_RESERVES = BigInt(
  bondingCurveConfig.initialVirtualTokenReserves,
);
const INITIAL_VIRTUAL_SOL_RESERVES = BigInt(
  bondingCurveConfig.initialVirtualSolReserves,
);
const INITIAL_REAL_SOL_RESERVES = BigInt(
  bondingCurveConfig.initialRealSolReserves,
);
...
export class BondingCurveService {
  ...
  private static readonly INITIAL_VIRTUAL_SOL_RESERVES = BigInt(
    bondingCurveConfig.initialVirtualSolReserves,
  ); // 30 SOL in lamports
  private static readonly INITIAL_REAL_TOKEN_RESERVES = 793100000; // ~793M tokens
  private static readonly TARGET_SOL_RAISED =
    bondingCurveConfig.graduationSolTarget; // This should be in lamports, consistent with sol_amount
   ...
   */
  static async calculateMarketCap(
    state: BondingCurveState,
    solPriceUSD?: number,
  ): Promise<number> {
    ...
    try {
      ...
      console.warn(
        "⚠️ [MARKET CAP] Failed to fetch SOL price, using fallback $169:",
        error,
      );
  ...
  static calculateProgress(state: BondingCurveState): BondingCurveProgress {
    ...
    const progress = Math.min(
      100,
      (tokensSold / totalBondingCurveTokens) * 100,
    );

    // Calculate SOL required for graduation
    const solRequired = this.calculateSolForTokens(
      state,
  ...
  static calculateBuyTrade(
    ...
    // Calculate new price
    const oldPrice = this.calculatePriceFromReserves(
      virtualSolReserves,
      virtualTokenReserves,
    );
    const newPrice = this.calculatePriceFromReserves(
  ...
  static async getTokenBondingCurveState(tokenId: string): Promise<{
    ...
    try {
      ...
      const progress = Math.min(
        (totalSolRaised / this.TARGET_SOL_RAISED) * 100,
        100,
      );
      ...
      const scaledVirtualTokenReserves =
        Number(INITIAL_VIRTUAL_TOKEN_RESERVES) * scalingFactor;
      const virtualSolReservesSOL =
        Number(this.INITIAL_VIRTUAL_SOL_RESERVES) / 1e9;
  ...
  static calculateSolForTokens(
    ...
    // Add platform fee
    const solWithFees =
      (solRequired * BigInt(BASIS_POINTS)) /
      BigInt(BASIS_POINTS - PLATFORM_FEE_BPS);
  ...
  static createInitialState(
    ...
    const scaledVirtualTokenReserves = BigInt(
      Math.floor(Number(INITIAL_VIRTUAL_TOKEN_RESERVES) * scalingFactor),
    );
