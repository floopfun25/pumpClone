  ...
  <div class="min-h-screen bg-binance-dark">
    ...
    <div v-else class="py-8">
      <div class="container mx-auto px-4 max-w-7xl">
        ...
        <div
          v-else
          class="bg-trading-surface border border-binance-border rounded-xl p-6 mb-6"
        >
          <div
          ...
          >
            ...
            <div class="flex items-center gap-4">
              <!-- Avatar -->
              <div
                class="w-24 h-24 rounded-full overflow-hidden bg-gradient-to-br from-binance-yellow to-primary-600 flex items-center justify-center text-binance-dark font-bold text-2xl relative"
              >
                <img
                ...
                <!-- Online Status -->
                <div
                  class="absolute bottom-1 right-1 w-6 h-6 bg-green-500 border-2 border-trading-surface rounded-full"
                ></div>
              </div>
        ...
        <!-- Tabs Navigation -->
        <div
          class="bg-trading-surface border border-binance-border rounded-xl mb-6"
        >
          <div class="flex border-b border-binance-border">
            ...
            >
              ...
              <span
                ...
                class="px-2 py-1 bg-binance-border text-xs rounded-full"
              >
                {{ tab.count }}
              </span>
            </button>
          ...
          <div class="p-6">
            ...
            <div v-if="activeTab === 'tokens'">
              ...
              <div v-else class="text-center py-12">
                ...
                <h3 class="text-xl font-semibold text-white mb-2">
                  {{ t("profile.noTokensCreated") }}
                </h3>
                <p class="text-binance-gray mb-4">
                  {{ t("profile.noTokensCreatedDesc") }}
                </p>
                <router-link
                  v-if="isOwnProfile"
                  to="/create"
            ...
            <div v-else-if="activeTab === 'watchlist'">
                  ...
                  <div class="space-y-2">
                    <div class="flex justify-between text-sm">
                      ...
                      <span class="text-white"
                        >${{ formatNumber(item.current_price || 0) }}</span
                      >
                    </div>
                    <div class="flex justify-between text-sm">
                      <span class="text-binance-gray"
                        >{{ t("token.marketCap") }}:</span
                      >
                      <span class="text-white"
                        >${{ formatNumber(item.market_cap || 0) }}</span
                      >
                    </div>
                    <div class="flex justify-between text-sm">
                      <span class="text-binance-gray"
                        >{{ t("token.volume24h") }}:</span
                      >
                      <span class="text-white"
                        >${{ formatNumber(item.volume_24h || 0) }}</span
                      >
                    </div>
                    <div class="flex justify-between text-sm">
                      <span class="text-binance-gray"
                        >{{ t("token.progress") }}:</span
                      >
                      <span class="text-binance-yellow"
                        >{{
                          (item.bonding_curve_progress || 0).toFixed(1)
                        }}%</span
                      >
                    </div>
              ...
              <div v-else class="text-center py-12">
                <div class="text-6xl mb-4">🤍</div>
                <h3 class="text-xl font-semibold text-white mb-2">
                  {{ t("profile.noWatchlistItems") }}
                </h3>
                <p class="text-binance-gray mb-4">
                  {{
                    loading
                      ? t("profile.loadingWatchlist")
                      : t("profile.addTokensToWatchlist")
                  }}
                </p>
            ...
            <div v-else-if="activeTab === 'history'">
              ...
              <div v-else class="text-center py-12">
                <div class="text-6xl mb-4">📈</div>
                <h3 class="text-xl font-semibold text-white mb-2">
                  {{ t("profile.noTradingHistory") }}
                </h3>
                <p class="text-binance-gray">
                  {{ t("profile.noTradingHistoryDesc") }}
                </p>
              </div>
            </div>
            ...
            <div v-else-if="activeTab === 'holdings'">
                ...
                >
                  ...
                  <div class="text-right">
                    ...
                    <div class="text-binance-gray text-sm">
                      {{
                        formatNumber(
                          holding.amount * (holding.token_price || 0),
                        )
                      }}
                      SOL
                    </div>
              ...
              <div v-else class="text-center py-12">
                <div class="text-6xl mb-4">💰</div>
                <h3 class="text-xl font-semibold text-white mb-2">
                  {{ t("profile.noHoldings") }}
                </h3>
                <p class="text-binance-gray">
                  {{ t("profile.noHoldingsDesc") }}
                </p>
              </div>
            </div>
            ...
            <div v-else-if="activeTab === 'activity'">
              <div v-if="userActivity.length > 0" class="space-y-3">
                ...
                >
                  <div
                    class="w-8 h-8 rounded-full bg-binance-yellow/20 flex items-center justify-center text-binance-yellow text-sm"
                  >
    ...
    >
      <div
        class="bg-trading-surface border border-binance-border rounded-xl p-6 max-w-md w-full mx-4"
      >
        ...
        <div class="space-y-4">
          <div>
            <label class="block text-sm font-medium text-binance-gray mb-2">{{
              t("profile.username")
            }}</label>
            <input
        ...
        <div class="flex gap-3 mt-6">
          ...
          <button @click="showEditModal = false" class="flex-1 btn-secondary">
            {{ t("common.cancel") }}
          </button>
        </div>
      </div>
...
const userName = computed(() => {
  return (
    user.value?.username ||
    formatWalletAddress(user.value?.wallet_address || "")
  );
});
...
const isOwnProfile = computed(() => {
  ...
  return (
    ...
    walletStore.walletAddress === route.params.address
  );
});
...
const calculateTotalHolders = (): number => {
  return userTokens.value.reduce(
    (total, token) => total + (token.holders || 0),
    0,
  );
};
...
const getActivityDescription = (activity: any): string => {
  switch (activity.type) {
    case "token_created":
      return t("profile.activityTypes.tokenCreated", {
        name: activity.token_name,
      });
    case "trade":
      return t("profile.activityTypes.trade", {
        type: activity.trade_type,
