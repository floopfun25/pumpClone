  ...
  >
    <div class="container mx-auto px-4 py-8">
      ...
      <div class="max-w-4xl mx-auto">
        <!-- Balance Card -->
        <div
          class="bg-white/10 backdrop-blur-md rounded-2xl p-6 mb-6 border border-white/20"
        >
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <!-- Fee Rate -->
            <div class="text-center">
              <div class="text-2xl font-bold text-purple-400 mb-2">1.0%</div>
              <div class="text-gray-300 text-sm">Platform Fee Rate</div>
              <div class="text-gray-400 text-xs mt-1">Per trade</div>
            </div>
          </div>
        </div>
          ...
          <div class="space-y-3">
            <div class="flex flex-col sm:flex-row sm:justify-between">
              ...
              <div class="flex items-center space-x-2">
                <code
                  class="bg-black/30 px-3 py-1 rounded text-green-400 text-sm break-all"
                >
                  {{ feeWalletAddress }}
                </code>
                <button
        ...
        >
          ...
          <div v-else-if="transactions.length === 0" class="text-center py-8">
            ...
            <div class="text-sm text-gray-500 mt-2">
              Start trading to see fees collected here!
            </div>
          </div>
        ...
        <!-- Testing Commands -->
        <div
          class="bg-white/10 backdrop-blur-md rounded-2xl p-6 mt-6 border border-white/20"
        >
          <h3 class="text-xl font-bold text-white mb-4">🧪 Testing Commands</h3>
          <div class="space-y-3">
            <div class="bg-black/30 rounded-lg p-4">
              <div class="text-gray-300 text-sm mb-2">
                Check balance via command line:
              </div>
            ...
            <div class="bg-black/30 rounded-lg p-4">
              ...
              <code class="text-green-400 text-sm break-all">
                solana transaction-history {{ feeWalletAddress }} --url
                {{ network }}
              </code>
            </div>
          </div>
...
const feeWalletAddress =
  import.meta.env.VITE_DEVNET_FEE_WALLET ||
  import.meta.env.VITE_MAINNET_FEE_WALLET;
...
const fetchTransactions = async () => {
  try {
    ...
    for (const sigInfo of signatures) {
      try {
        ...
        if (tx && tx.meta && !tx.meta.err) {
          ...
          if (change > 0) {
            // Only show incoming transactions (fees)
            txDetails.push({
    ...
    totalFeesCollected.value = txDetails.reduce(
