    ...
    <section id="tokens" class="py-3 bg-trading-surface">
      <div class="container mx-auto px-4">
        ...
        >
          <div>
            <h2 class="text-lg font-bold text-white">
              {{ t("common.all") }} {{ t("token.tokens") }}
            </h2>
            <!-- Pagination Info -->
            <div
              v-if="tokens.length > 0"
              class="text-binance-gray text-xs mt-2 flex items-center gap-4"
            >
              <span
                >{{ t("pagination.showing") }}
                {{ currentPageInfo.currentCount }} {{ t("token.tokens") }}</span
              >
              <span v-if="currentPageInfo.currentPage > 1"
                >{{ t("pagination.page") }} {{ currentPageInfo.currentPage }}
                {{ t("pagination.of") }} {{ currentPageInfo.totalPages }}</span
              >
              <span
                v-if="currentPageInfo.hasNextPage"
                class="text-binance-yellow"
                >{{ t("pagination.moreAvailable") }}</span
              >
            </div>
          </div>
          ...
          <div
            class="filter-buttons hidden md:flex flex-wrap gap-2 mt-4 md:mt-0"
          >
            <button
              v-for="filter in quickFilters"
              :key="filter.value"
        ...
        <div v-if="tokens.length > 0" class="relative">
          ...
          <div
            v-if="loading"
            class="absolute inset-0 bg-trading-surface bg-opacity-75 flex items-center justify-center z-10 rounded-lg"
          >
            <div
              class="flex items-center gap-3 bg-trading-elevated px-6 py-3 rounded-lg border border-binance-border"
            >
              <div class="spinner w-5 h-5"></div>
              <span class="text-white text-sm"
                >{{ t("pagination.loading") }}...</span
              >
            </div>
          </div>

          <div
            class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-3"
        ...
        <div v-else-if="error" class="text-center py-12">
          ...
          <h3 class="text-xl font-semibold text-red-400 mb-2">
            {{ t("messages.error.loadTokens") || "Error Loading Tokens" }}
          </h3>
        ...
        <!-- Page Navigation -->
        <div
          v-if="tokens.length > 0 && totalPages > 1"
          class="pagination-controls flex items-center justify-center mt-8 gap-4"
        >
          <!-- Previous Button -->
          <button
            @click="goToPrevPage"
            ...
            :class="{
              'opacity-50 cursor-not-allowed':
                !currentPageInfo.hasPrevPage || loading,
            }"
          >
            <svg
              class="h-4 w-4"
          ...
          <!-- Page Info -->
          <div
            class="px-4 py-2 bg-trading-elevated rounded-lg border border-binance-border"
          >
            <span class="text-white text-sm">
              {{ t("pagination.page") }} {{ currentPageInfo.currentPage }}
              {{ t("pagination.of") }} {{ currentPageInfo.totalPages }}
            </span>
          </div>
          ...
          <button
            ...
            :class="{
              'opacity-50 cursor-not-allowed':
                !currentPageInfo.hasNextPage || loading,
            }"
          >
            {{ t("pagination.next") }}
            <svg
  ...
  try {
    ...
    error.value =
      loadError instanceof Error
        ? loadError.message
        : "Failed to load token data. Please check your internet connection and try again.";
