          ...
          <div v-else class="overflow-x-auto">
            <table class="w-full">
              ...
              <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                <tr
                  v-for="holding in portfolioData.tokenHoldings"
                  :key="holding.mint"
                ...
                >
                  ...
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="flex items-center">
                      <img
                        :src="
                          holding.metadata?.image || getTokenFallbackImage()
                        "
                      ...
                      <div class="ml-3">
                        ...
                        >
                          ...
                          <span
                            v-if="holding.metadata?.verified"
                            class="text-xs bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400 px-1.5 py-0.5 rounded-full"
                          >
                            ✓
                          </span>
                        </p>
                        <p class="text-sm text-gray-500 dark:text-gray-400">
                          ...
                          <span
                            v-if="
                              ...
                              holding.metadata.source !== 'database'
                            "
...
const totalPnL = computed(() => {
  ...
  return (
    portfolioData.value.tokenHoldings.reduce(
      (sum, token) => sum + token.priceChange24h,
      0,
    ) / portfolioData.value.tokenHoldings.length
  );
});
...
const bestPerformer = computed(() => {
  ...
  const best = portfolioData.value.tokenHoldings.reduce(
    (best, current) =>
      current.priceChange24h > best.priceChange24h ? current : best,
    portfolioData.value.tokenHoldings[0],
  );
...
const loadPortfolio = async (): Promise<void> => {
  ...
  try {
    ...
    try {
      portfolioValue =
        await priceOracleService.calculatePortfolioValue(holdings);
    } catch (priceError) {
      console.error("Failed to calculate portfolio value:", priceError);
      throw new Error(
        `Unable to load portfolio data: ${priceError instanceof Error ? priceError.message : "Unknown error"}. Please check your internet connection and try again.`,
      );
    }
