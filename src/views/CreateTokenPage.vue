          ...
          >
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              ...
              <div>
                ...
                >
                  <option value="0">
                    {{ t("token.creatorShareOptions.0") }}
                  </option>
                  <option value="10">
                    {{ t("token.creatorShareOptions.10") }}
                  </option>
                  <option value="20">
                    {{ t("token.creatorShareOptions.20") }}
                  </option>
                  <option value="30">
                    {{ t("token.creatorShareOptions.30") }}
                  </option>
                  <option value="40">
                    {{ t("token.creatorShareOptions.40") }}
                  </option>
                  <option value="50">
                    {{ t("token.creatorShareOptions.50") }}
                  </option>
                  <option value="60">
                    {{ t("token.creatorShareOptions.60") }}
                  </option>
                  <option value="70">
                    {{ t("token.creatorShareOptions.70") }}
                  </option>
                  <option value="80">
                    {{ t("token.creatorShareOptions.80") }}
                  </option>
                </select>
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
            ...
            >
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <select
                    v-model="tokenForm.lockDurationDays"
                    class="input-field"
                  >
              ...
              <div class="grid grid-cols-2 gap-4 text-sm">
                <div>
                  <span class="text-gray-600 dark:text-gray-400"
                    >{{ t("token.totalSupply") }}:</span
                  >
                  <span
                    class="font-medium text-gray-900 dark:text-white ml-2"
                    >{{ formatNumber(tokenCreationSummary.totalSupply) }}</span
                  >
                </div>
                <div>
                  <span class="text-gray-600 dark:text-gray-400"
                    >{{ t("token.marketSupply") }}:</span
                  >
                  <span
                    class="font-medium text-gray-900 dark:text-white ml-2"
                    >{{ formatNumber(tokenCreationSummary.marketSupply) }}</span
                  >
                </div>
                <div>
                  <span class="text-gray-600 dark:text-gray-400"
                    >{{ t("token.creatorTokens") }}:</span
                  >
                  <span
                    class="font-medium text-gray-900 dark:text-white ml-2"
                    >{{
                      formatNumber(tokenCreationSummary.creatorTokens)
                    }}</span
                  >
                </div>
                <div>
                  <span class="text-gray-600 dark:text-gray-400"
                    >{{ t("token.lockedTokens") }}:</span
                  >
                  <span
                    class="font-medium text-gray-900 dark:text-white ml-2"
                    >{{ formatNumber(tokenCreationSummary.lockedTokens) }}</span
                  >
                </div>
              </div>
            </div>
          ...
          >
            ...
            <div>
              ...
              <div v-if="prebuyEstimate" class="mt-3 text-sm">
                <span class="text-gray-600 dark:text-gray-400"
                  >{{ t("token.estimatedTokens") }}:</span
                >
                <span
                  class="font-medium text-green-600 dark:text-green-400 ml-2"
                >
                  {{ formatNumber(prebuyEstimate.tokens) }}
                  {{ tokenForm.symbol }}
                </span>
              </div>
            </div>
          ...
          >
            <div class="flex items-center justify-between">
              ...
              <div class="text-right text-xs text-blue-600 dark:text-blue-400">
                <div>
                  {{ t("token.platform") }}: {{ creationCost.creationFee }} SOL
                </div>
                <div>
                  {{ t("token.rent") }}:
                  {{ creationCost.rentExempt.toFixed(4) }} SOL
                </div>
                <div>
                  {{ t("token.gas") }}: {{ creationCost.transaction }} SOL
                </div>
              </div>
            </div>
          </div>
...
const prebuyEstimate = computed(() => {
  if (!tokenForm.value.prebuyAmount || !tokenForm.value.totalSupply)
    return null;
...
const validateAdvancedFields = (): string[] => {
  ...
  if (
    tokenForm.value.totalSupply &&
    (tokenForm.value.totalSupply < 100000 ||
      tokenForm.value.totalSupply > 1000000000)
  ) {
  ...
  if (
    tokenForm.value.lockPercentage &&
    (tokenForm.value.lockPercentage < 0 || tokenForm.value.lockPercentage > 100)
  ) {
    errors.push(t("token.errors.invalidLockPercentage"));
  }

  if (
    tokenForm.value.prebuyAmount &&
    (tokenForm.value.prebuyAmount < 0 || tokenForm.value.prebuyAmount > 10)
  ) {
...
onMounted(() => {
  fileInput.value =
    document.querySelector<HTMLInputElement>('input[type="file"]') || undefined;
});
</script>
